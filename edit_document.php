	...
	if( $_POST ) {
		...
		if( !$isBlank ) {
			if( isset( $_FILES["flImage"] ) and $_FILES["flImage"] != ''  ) {
				...
				if( move_uploaded_file( $_FILES["flImage"]["tmp_name"], "upload/docs/" . $fileName ) ) {
					if($size>50){
						$isValidUpload = false;
						$fileName = $_POST['hid_filename'];
					}else{
						if($oldPic!=''){
							$link="upload/docs/".$oldPic;
				...
				if( $isValidUpload===true ){	
					...
					"UPDATE tbl_docs SET 
						doc_name='".$fileName."'
						WHERE doc_id ='".$_POST['prd_id']."'" ) or die(mysql_error());
								...
								<input type="hidden" name="prd_id" id="prd_id" value="<?=$prd_id?>" />
									...
									<div class="col-md-6">
										...
										<div style="margin-bottom:10px;">
											<b>Document Name :</b> <?php echo $logo;?>
										</div>
