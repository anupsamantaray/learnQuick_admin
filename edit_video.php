	...
	if( $_POST ) {
		...
		if( !$isBlank ) {
			if( isset( $_FILES["flImage"] ) and $_FILES["flImage"] != ''  ) {
				...
				if( move_uploaded_file( $_FILES["flImage"]["tmp_name"], "upload/videos/" . $fileName ) ) {
					
					//list($width, $height) = getimagesize( "upload/videos/".$fileName );
				...
				if( $isValidUpload===true ){	
					...
					"UPDATE tbl_video SET 
						video_name='".$fileName."'
						WHERE video_id ='".$_POST['prd_id']."'" ) or die(mysql_error());
		...
		if( isset( $prd_id ) ) {
			$q = "SELECT * FROM  tbl_video WHERE video_id='".$prd_id."'";
			$QUERY_PRODUCT = mysql_query( $q );
			$RESULT = mysql_fetch_assoc( $QUERY_PRODUCT );
			$logo			= $RESULT["video_name"];
